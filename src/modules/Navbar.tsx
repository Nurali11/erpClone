...
const Navbar = () => {
  ...
  return (
    <div className={`${showNavbar ? "w-[87px]" :"w-[18%]"} duration-300  h-[100vh] overflow-hidden bg-[#031529]`}>
      <div className={`p-2 border-b-[0.5px] px-[15px] border-slate-500 flex ${showNavbar ? "justify-center" :"gap-[10px]"} items-center`}>
        ...
        <p className={`${showNavbar ? "opacity-0 w-0" : ""} duration-300 capitalize text-[25px] text-white`}>{role.toLowerCase()}</p>
      </div>
      <NavbarCarusel/>
    </div>
  )
